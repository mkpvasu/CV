...
image make_integral_image(image im)
{
    ...
    for(k = 0; k<im.c; k++){
    integ.data[0] = im.data[0];
    for(i = 1; i<im.w; i++){
    integ.data[k*im.h*im.w + i] = im.data[k*im.h*im.w + i] + integ.data[k*im.h*im.w + i-1];
    }
    for(j = 1; j<im.h; j++){
    integ.data[k*im.h*im.w + j*im.w] = im.data[k*im.h*im.w + j*im.w] + integ.data[k*im.h*im.w + (j-1)*im.w];
    }
    }
    for(k = 0; k<im.c; k++){
	for(j = 1; j<im.h; j++){
	   for(i = 1; i<im.w; i++){
		index = k*im.w*im.h + j*im.w + i;
		integ.data[index] = im.data[index] + integ.data[index-1] + integ.data[index-im.w] - integ.data[index-im.w-1];
	    }
	 }
     }
...
image time_structure_matrix(image im, image prev, int s)
{
	...
	for(i = 0; i<im.w*im.h; i++){
	    S.data[i] = powf(Ix.data[i],2);
            S.data[i + 1*S.w*S.h] = powf(Iy.data[i],2);
            S.data[i + 2*S.w*S.h] = Ix.data[i]*Iy.data[i];
            S.data[i + 3*S.w*S.h] = Ix.data[i]*It.data[i];
            S.data[i + 4*S.w*S.h] = Iy.data[i]*It.data[i];
	}
    ...
    free_image(gx);free_image(gy);free_image(Ix);free_image(Iy);free_image(It);
    return S;
}
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
	    ...
	    memcpy(*(M.data),a[0],sizeof(a[0]));
	    memcpy(*(M.data+1),a[1],sizeof(a[1]));
	    
	    matrix M_in = matrix_invert(M);
	    if(M_in.data){
		matrix V = matrix_mult_matrix(M_in,X);
		vx = V.data[0][0];
		vy = V.data[1][0];
	     }
