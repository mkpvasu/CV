...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
	...
	for(i = 0; i<bn; i++){
	float cdist = l1_distance(a[j].data, b[i].data, a[j].n);
	if(m[j].distance > cdist){ m[j].distance = cdist; bind = i; }
	}
        m[j].ai = j;
        m[j].bi = bind; // <- should be index in b.
        m[j].p = a[j].p;
        m[j].q = b[bind].p;
        // m[j].distance // <- should be the smallest L1 distance!
    }
...
matrix multiply(matrix H, matrix c)
{
matrix result = make_matrix(3,1);
if(H.cols == c.rows){
   // Multiplying first and second matrices and storing it in result
   for (int i = 0; i < H.rows; ++i) {
      for (int j = 0; j < c.cols; ++j) {
         for (int k = 0; k < H.cols; ++k) {
            result.data[i][j] += H.data[i][k] * c.data[k][j];
         }
      }
   }
}
...
float point_distance(point p, point q)
{
	...
	float val = sqrt(powf((p.x - q.x),2) + powf((p.y - q.y),2));
    return val;
}
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
	...
	for(i = 0; i<n; i++){
	point l = project_point(H,m[i].p);
	float pdist = point_distance(l,m[i].q);
	if(pdist<thresh){ count += 1; }
	else{
	match swap = m[count];
...
matrix compute_homography(match *matches, int n)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        double a1[8] = {x, y, 1, 0, 0, 0, -x*xp, -y*xp};
        double a2[8] = {0, 0, 0, x, y, 1, -x*yp, -y*yp};
        memcpy(*(M.data+2*i),   a1, sizeof(a1));
        memcpy(*(M.data+2*i+1), a2, sizeof(a2));
        b.data[2*i][0] = xp;
        b.data[2*i+1][0] = yp;
    }
    ...
    for(k = 0; k < b.c; ++k){
        for(j = topleft.y; j < botright.y; ++j){
            for(i = topleft.x; i < botright.x; ++i){
                // TODO: fill in.
		point g = project_point(H, make_point(i,j));
		if(g.x >= 0 && g.x < a.w && g.y >= 0 && g.y < a.h ){
		float val = bilinear_interpolate(b, g.x, g.y, k);
